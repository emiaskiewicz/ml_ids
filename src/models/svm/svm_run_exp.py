...
CONFIG_PATH = PROJECT_DIR / "config" / "svm.yaml"
SCRIPT_PATH = PROJECT_DIR / "src" / "models" / "svm" / "svm_model.py"
DATASET_VARIANT = "easy"
LOG_PATH = PROJECT_DIR / "logs" / "svm" / DATASET_VARIANT / "runner.log"
STOP_ON_ERROR = True
SKIP_FINISHED = True
SAVE_PLOTS = False
EVALUATE_TEST = False
...
def apply_experiment_settings(config: dict, exp: dict) -> dict:
    ...
    config["output"]["output_dir"] = f"outputs/svm/{DATASET_VARIANT}/{exp_id}"
    config["output"]["summary_path"] = f"outputs/svm/{DATASET_VARIANT}/svm_{DATASET_VARIANT}_results_sum.csv"
    config["logging"]["log_path"] = f"logs/svm/{DATASET_VARIANT}/{exp_id}.log"
