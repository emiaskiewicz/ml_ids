...
CONFIG_PATH = PROJECT_DIR / "config" / "decision_tree.yaml"
SCRIPT_PATH = PROJECT_DIR / "src" / "models" / "decision_tree" / "dt_model.py"
DATASET_VARIANT = "easy"
LOG_PATH = PROJECT_DIR / "logs" / "decision_tree" / DATASET_VARIANT / "runner.log"
STOP_ON_ERROR = True
SKIP_FINISHED = True
SAVE_PLOTS = False
EVALUATE_TEST = False
...
def apply_experiment_settings(config: dict, exp: dict) -> dict:
    ...
    config["output"]["output_dir"] = f"outputs/decision_tree/{DATASET_VARIANT}/{exp_id}"
    config["output"]["summary_path"] = f"outputs/decision_tree/{DATASET_VARIANT}/dt_{DATASET_VARIANT}_results_sum.csv"
    config["logging"]["log_path"] = f"logs/decision_tree/{DATASET_VARIANT}/{exp_id}.log"
...
def main() -> None:
    ...
    for index, exp in enumerate(experiments, start=1):
        ...
        print(f"Corr={exp['correlation']}, FS={exp['feature_selection']}, SMOTE={exp['smote']}, "
